def Cybergarden::Commands.upgrade(cybergarden : Cybergarden::Client,
  ...
  meets_requirements = money >= garden.get_next_upgrade_requirement

  if !meets_requirements
    content = "X - You don't have enough money to do so! (#{money.humanize(precision: 2)} out of #{garden.get_next_upgrade_requirement.humanize(precision: 2)})"
